...
def auth_vk_id():
    ...
    tokens = vk_id_auth.exchange_code_for_tokens(code, state, device_id, code_verifier)
    access_token = tokens['access_token']
    
    res = requests.post(config.API_URL + "/v1/auth/vk-id", json={"access_token": access_token}).json()
